...
KILL_POINTS = 100 # changed KILL_POINTS
...
class GameState:
  ...
  def makeObservation(self, index):
    ...
    # Adds the sonar signal, update distance to enemies
...
class CaptureRules:
  ...
  def process(self, state, game):
    ...
    """
    # No time limit for running game
  ...
  def applyAction( state, action, agentIndex ):
    ...
    if next == nearest:
      ...
      if agentState.numCarrying > 0 and not agentState.isPacman:
        score = agentState.numCarrying if isRed else 0 # -1*agentState.numCarrying
...
def readCommand( argv ):
  ...
  parser.add_option('-r', '--red', help=default('Red team'),
                    default='PacmanDQNTeam')#default='baselineTeam')
  parser.add_option('-b', '--blue', help=default('Blue team'),
                    default='baselineTeam')#default='baselineTeam')
  ...
  parser.add_option('-n', '--numGames', type='int',
                    help=default('Number of games to play'), default=1010)  # default = 1
  parser.add_option('-f', '--fixRandomSeed', action='store_true',
                    help='Fixes the random seed to always play the same game', default=False)
  parser.add_option('--record', action='store_true',
                    help='Writes game histories to a file (named by the time they were played)', default=True)#default=False)
  parser.add_option('--replay', default=None,
                    help='Replays a recorded game file.')
  parser.add_option('-x', '--numTraining', dest='numTraining', type='int',
                    help=default('How many episodes are training (suppresses output)'), default=1000) # default = 0
  ...
  parser.add_option('-m','--model_file', dest='model_file', type='str',
                     help=default('Model file to load'),default=None)
